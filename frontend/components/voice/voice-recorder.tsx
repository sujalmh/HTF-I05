...
export function VoiceRecorder({ onTranscript, onClose, open }: VoiceRecorderProps) {
  ...
  const sendToBackend = async (audioBlob: Blob) => {
    ...
    try {
      // Convert to mp3 using webm or wav is ok, backend will handle the type
      const formData = new FormData()
      formData.append("file", audioBlob, "recording.wav") // or "recording.webm"
          ...
          <div className="flex flex-col items-center justify-center gap-4">
            <div
              className={`w-24 h-24 rounded-full flex items-center justify-center ${
                isRecording ? "bg-red-100 animate-pulse" : audioUrl ? "bg-blue-100" : "bg-gray-100"
              }`}
            >
              <Mic
                className={`h-10 w-10 ${isRecording ? "text-red-500" : audioUrl ? "text-blue-500" : "text-gray-400"}`}
              />
            </div>
            ...
            {isRecording ? (
              ...
              <div className="text-center">
                <div className="text-xl font-semibold">Recording Complete</div>
                <div className="text-sm text-gray-500">{formatTime(recordingTime)}</div>
              </div>
            ) : (
              <div className="text-center">
                <div className="text-xl font-semibold">Ready to Record</div>
                <div className="text-sm text-gray-500">Click the button below to start</div>
              </div>
            )}
            ...
            <div className="mt-6 border rounded-lg p-4">
              ...
              {isProcessing ? (
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-gray-500 text-center">Processing audio...</p>
                </div>
              ) : (
